...
/* $ModDesc: Implements channel mode +z and extban 'z' - moderator mute */
/* $ModDepends: core 2.0 */

class OpModeratedMode : public SimpleChannelModeHandler
{
 public:
	OpModeratedMode(Module* Creator) : SimpleChannelModeHandler(Creator, "opmoderated", 'z') { }
};
...
class ModuleOpModerated : public Module
{
 ...
 public:
	...
	Version GetVersion()
	{
		return Version("Implements opmoderated channel mode +z (non-voiced messages sent to ops) and extban 'z'", VF_OPTCOMMON|VF_VENDOR);
	}
	...
	ModResult DoMsg(User *user, void *dest, int target_type, std::string &text, char status, CUList &exempt_list, bool privmsg)
	{
		...
		if (res == MOD_RES_ALLOW)
			return MOD_RES_PASSTHRU;
		if (!chan->GetExtBanStatus(user, 'z').check(!chan->IsModeSet(&mh)) && chan->GetPrefixValue(user) < VOICE_VALUE)
		{
